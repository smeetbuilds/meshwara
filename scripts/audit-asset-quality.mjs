...
for (const block of blocks) {
  const asset = {
    ...
    subcategory: field(block, 'subcategory', false) ?? null,
    scene: field(block, 'scene'),
    sourceType: field(block, 'sourceType'),
    complexity: field(block, 'complexity'),
    interaction: field(block, 'interaction'),
    presentation: field(block, 'presentation', false) ?? 'Floating',
  }

  if (asset.sourceType === 'Model') {
    records.push({ ...asset, audit: 'MODELED_GATE', criticalPassed: true })
    continue
  }
  ...
  const checks = {
    deterministic,
    placeholderFree,
    authoredSource: bytes >= 600,
    renderable: effectiveGeometry >= 1 || shaderDriven,
    materialDefined: materials >= 1,
    premiumForCinematic: asset.complexity !== 'Cinematic' || premiumMaterials >= 1,
  ...
  const score = Math.round(Math.min(100,
    35 +
    Math.min(18, bytes / 120) +
    Math.min(22, effectiveGeometry * 1.45) +
    Math.min(10, materials * 2) +
    Math.min(5, distinctColors * 1.25) +
    (premiumMaterials ? 5 : 0) +
    (animated || shaderDriven ? 5 : 0)
  ))

  records.push({
    ...asset,
    criticalPassed: Object.values(checks).every(Boolean),
    score,
    metrics: { bytes, meshLike, geometries, repeatedGeometryEstimate: repeated, effectiveGeometry, materials, premiumMaterials, distinctColors, animated, shaderDriven },
    checks,
  })
}
...
const report = {
  brand: 'MESHVARA',
  generatedBy: 'scripts/audit-asset-quality.mjs',
  assetCount: records.length,
  criticalPassed: records.every((record) => record.criticalPassed),
  minimumAutomatedScore: Math.min(...scores),
  averageAutomatedScore: Number((scores.reduce((sum, value) => sum + value, 0) / scores.length).toFixed(2)),
  note: 'Automated checks verify source depth, deterministic construction, rendering/material contracts, semantic density and packaging readiness. They complement — not replace — visual art-direction review.',
  assets: records,
}
