...
const assets = blocks.map((block) => ({
  ...
  index: Number(field(block, 'index')),
  name: field(block, 'name'),
  category: field(block, 'category'),
  subcategory: field(block, 'subcategory', false),
  scene: field(block, 'scene'),
  blurb: field(block, 'blurb'),
  description: field(block, 'description'),
  tags: [...block.matchAll(/tags:\s*\[([^\]]+)\]/g)].flatMap((m) => [...m[1].matchAll(/'([^']+)'/g)].map((x) => x[1])),
...
for (const asset of assets) {
  ...
  // New high-volume production batches carry a stricter authored-source floor.
  if (asset.index >= 201) {
  ...
  if (asset.index >= 301) {
    ...
    if (geometryCount < 9) failures.push(`${asset.slug}: 301+ production scene requires at least 9 authored geometry nodes (${geometryCount})`)
    ...
    if (asset.category === 'Animals' && geometryCount < 12) failures.push(`${asset.slug}: animal study requires at least 12 authored anatomy/detail nodes (${geometryCount})`)
    if ((asset.category === 'Architecture' || asset.category === 'Scenes') && geometryCount < 12) failures.push(`${asset.slug}: spatial asset requires at least 12 authored composition nodes (${geometryCount})`)
  }


  const normalized = scene
    .replace(/function\s+[A-Za-z0-9_]+/g, 'function Scene')
    .replace(/export\s+default\s+[A-Za-z0-9_]+/g, 'export default Scene')
    .replace(/\s+/g, ' ')
    .trim()
