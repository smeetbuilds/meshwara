...
const [
  ...
  exportModule, modelTools, modelExport, textureResources, componentPack, browserZip, themeModule, themeCss, codecCapabilities,
...
for (const token of ['studioUnsupportedRequiredGlbExtensions', 'KHR_draco_mesh_compression', 'EXT_meshopt_compression', 'KHR_texture_basisu', 'assertStudioGlbCapabilities']) assert(codecCapabilities.includes(token), `codec capability gate missing ${token}`)
assert(storage.includes("import { assertStudioGlbCapabilities } from './studioGlbCapabilities'") && storage.includes('assertStudioGlbCapabilities(json)'), 'GLB storage validation does not enforce codec capability requirements')
...
assert(library.includes('IMPORT LOCAL GLB') && library.includes('PROJECTS') && library.includes('assets.filter') && library.includes('required Draco / Meshopt / KTX2 codecs are detected before local storage'), 'asset/project library is incomplete')
...
for (const token of [
  ...
  'rejects unsupported required codec GLBs', "page.on('pageerror'", "message.type() === 'error'", "ControlOrMeta+D", "ControlOrMeta+Z",
...
assert(contract.includes('KHR_draco_mesh_compression') && contract.includes('EXT_meshopt_compression') && contract.includes('KHR_texture_basisu'), 'codec capability detection contract is not explicit')
...
console.log('Meshvara Studio codec + advanced browser release contract passed')
