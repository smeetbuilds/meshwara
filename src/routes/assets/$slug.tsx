    ...
    <article className="detail-page" style={{ '--asset-accent': asset.accent } as CSSProperties}>
      ...
      <section className="asset-info page-pad" id="asset-info">
        <div className="info-main">
          <p className="section-label">OBJECT / {asset.index}</p>
          <h2>{asset.description}</h2>
          <div className="tag-row">{asset.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
        </div>
      ...
      <section className="related page-pad">
        ...
        <div className="related-links">
          {related.length ? related.map((item) => (
            <Link key={item.slug} to="/assets/$slug" params={{ slug: item.slug }}>
              <span>{item.index}</span><strong>{item.name}</strong><ArrowUpRight />
            </Link>
          )) : <Link to="/assets"><span>ALL</span><strong>Browse the full library</strong><ArrowUpRight /></Link>}
        </div>
      </section>
    </article>
  )
