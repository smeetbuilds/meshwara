...
function Studio({ compact }: { compact: boolean }) {
  return (
    <>
      <hemisphereLight color="#fffdf8" groundColor="#a8a49b" intensity={compact ? 1.05 : 1.15} />
      <directionalLight position={[5.5, 6.5, 4.5]} intensity={compact ? 2.15 : 2.6} />
      <directionalLight position={[-4.5, 2.2, -3.5]} intensity={compact ? 1.05 : 1.3} />
      <pointLight position={[0, -1.6, 3.6]} intensity={compact ? 0.35 : 0.55} distance={8} />
      <Environment resolution={compact ? 96 : 160}>
...
function FramedScene({ children, compact }: { children: ReactNode; compact: boolean }) {
  return (
    <Bounds fit clip observe margin={compact ? 1.3 : 1.16}>
      {children}
    </Bounds>
  )
}

export function AssetScene({ kind, compact = false, presentation = 'Floating', interaction = 'Pointer' }: { kind: AssetSceneKind; compact?: boolean; presentation?: AssetPresentation; interaction?: AssetInteraction }) {
  const Scene = sceneRegistry[kind]
  const reducedMotion = useReducedMotion()
  const content = presentation === 'Floating' ? (
    <Rig enabled={!reducedMotion && interaction === 'Pointer'}>
      <Float speed={reducedMotion ? 0 : compact ? 0.56 : 0.78} rotationIntensity={reducedMotion ? 0 : 0.065} floatIntensity={reducedMotion ? 0 : 0.11}>
        <Scene />
      </Float>
    </Rig>
  ...
  return (
    <Canvas
      ...
      onCreated={({ gl }) => {
        ...
        gl.toneMappingExposure = compact ? 1.03 : 1.08
