...
import { assetCustomizationRanges, getAssetCustomizationDefinitionByAssetSlug, identifyAssetCustomizationPreset, type AssetCustomization } from '../../lib/assetCustomization'
    ...
    <div className="studio-vector">
      ...
      <div>
        {axes.map((axis, index) => (
          <label key={axis}>
            <small>{axis}</small>
            <input type="number" step={step} value={Number(value[index].toFixed(4))} onChange={(event) => {
...
function TextureRow({ channel, authored, reference, onImport, onReference }: {
  ...
  return (
    <div className="studio-texture-row">
      ...
      <label className="studio-texture-upload">
        ...
        <input type="file" accept="image/png,image/jpeg,image/webp,.png,.jpg,.jpeg,.webp" onChange={(event) => {
...
function MaterialEditor({ slot, override, onPatch, onTextureImport, onTextureReference }: {
    ...
    <details className="studio-material-card">
      ...
      <div className="studio-material-controls">
        ...
        <div className="studio-texture-editor">
          ...
          {studioEditableTextureChannels.map((channel) => <TextureRow key={channel} channel={channel} authored={slot.textureChannels.includes(channel)} reference={override?.textures?.[channel]} onImport={(file) => onTextureImport(channel, file)} onReference={(reference) => onTextureReference(channel, reference)} />)}
        </div>
...
function ModelReport({ inspection }: { inspection: StudioModelInspection }) {
  ...
  return (
    <div className="studio-model-report">
      ...
      <div className="studio-stat-grid"><span><b>{inspection.meshes}</b> meshes</span><span><b>{inspection.triangles.toLocaleString()}</b> tris</span><span><b>{inspection.vertices.toLocaleString()}</b> verts</span><span><b>{inspection.materials}</b> materials</span><span><b>{inspection.textures}</b> textures</span><span><b>{inspection.skinnedMeshes}</b> skinned</span></div>
...
function ArchiveCustomizationEditor({ node, onCustomizationPatch }: { node: StudioNode; onCustomizationPatch: (customization: AssetCustomization) => void }) {
  const definition = node.assetSlug ? getAssetCustomizationDefinitionByAssetSlug(node.assetSlug) : null
  if (!definition) return null
  const customization = node.customization
  const activePreset = identifyAssetCustomizationPreset(definition.scene, customization)
  const patch = (next: Partial<AssetCustomization>) => onCustomizationPatch({ ...customization, ...next })
  const range = (field: 'roughnessScale' | 'metalnessScale' | 'emissiveScale' | 'opacity' | 'geometryScale', label: string) => {
    const [min, max, step] = assetCustomizationRanges[field]
    return <label className="studio-range-field"><span>{label} <output>{customization[field].toFixed(2)}</output></span><input type="range" min={min} max={max} step={step} value={customization[field]} onChange={(event) => patch({ [field]: Number(event.currentTarget.value) })} /></label>
  }
  return (
    <div className="studio-subsection studio-archive-customization">
      <div className="studio-section-title"><span>TYPED CUSTOMIZATION</span><small>PACK PARITY</small></div>
      <p className="studio-action-note">The same typed values drive Playground, Studio, CUSTOMIZATION.json and the downloadable component API.</p>
      <label className="studio-field"><span>Preset</span><select value={activePreset ?? 'custom'} onChange={(event) => {
        const preset = definition.presets.find((item) => item.id === event.currentTarget.value)
        if (preset) onCustomizationPatch({ ...preset.value })
      }}><option value="custom" disabled>CUSTOM</option>{definition.presets.map((preset) => <option key={preset.id} value={preset.id}>{preset.label.toUpperCase()}</option>)}</select></label>
      <label className="studio-field"><span>Palette</span><select value={customization.palette} onChange={(event) => patch({ palette: event.currentTarget.value as AssetCustomization['palette'] })}><option value="authored">AUTHORED</option><option value="mono">MONO</option><option value="duotone">DUOTONE</option></select></label>
      {customization.palette !== 'authored' ? <div className="studio-customization-colors"><label className="studio-mini-color"><span>Primary</span><input type="color" value={customization.primaryColor} onChange={(event) => patch({ primaryColor: event.currentTarget.value })} /></label><label className="studio-mini-color"><span>Secondary</span><input type="color" value={customization.secondaryColor} disabled={customization.palette === 'mono'} onChange={(event) => patch({ secondaryColor: event.currentTarget.value })} /></label></div> : null}
      {range('roughnessScale', 'Roughness scale')}
      {range('metalnessScale', 'Metalness scale')}
      {range('emissiveScale', 'Emissive scale')}
      {range('opacity', 'Opacity')}
      {range('geometryScale', 'Object scale')}
      <div className="studio-toggle-grid"><label><input type="checkbox" checked={customization.wireframe} onChange={(event) => patch({ wireframe: event.currentTarget.checked })} /> Wireframe</label></div>
      <button className="studio-material-reset" type="button" onClick={() => onCustomizationPatch({ ...definition.defaults })}>RESET TO AUTHORED</button>
    </div>
  )
}

export function StudioInspector({ project, node, selectedIds, inspection, onRename, onTransform, onNodePatch, onScenePatch, onParent, onMaterialPatch, onTextureImport, onTextureReference, onAnimationPatch, onDebugPatch, onBulkPatch, onDuplicate, onDelete, onExportGlb, onExportComponent }: {
  ...
  onNodePatch: (patch: Partial<Pick<StudioNode, 'visible' | 'locked' | 'wireframe' | 'customization'>>) => void
  ...
  return (
    <aside className="studio-inspector">
      <section className="studio-panel-section">
        ...
        {selectedIds.length > 1 ? <div className="studio-multi-card"><strong>{selectedIds.length} OBJECTS SELECTED</strong><div><button type="button" onClick={() => onBulkPatch({ visible: true })}>SHOW</button><button type="button" onClick={() => onBulkPatch({ visible: false })}>HIDE</button><button type="button" onClick={() => onBulkPatch({ locked: true })}>LOCK</button><button type="button" onClick={() => onBulkPatch({ locked: false })}>UNLOCK</button></div><small>Primary gizmo deltas are committed across selected root objects while preserving hierarchy.</small></div> : null}
        {node ? <>
          ...
          <div className="studio-toggle-grid"><label><input type="checkbox" checked={node.visible} onChange={(event) => onNodePatch({ visible: event.currentTarget.checked })} /> Visible</label><label><input type="checkbox" checked={node.locked} onChange={(event) => onNodePatch({ locked: event.currentTarget.checked })} /> Locked</label><label><input type="checkbox" checked={node.wireframe} onChange={(event) => onNodePatch({ wireframe: event.currentTarget.checked })} /> Wireframe</label></div>

          {node.kind === 'archive' ? <ArchiveCustomizationEditor node={node} onCustomizationPatch={(customization) => onNodePatch({ customization })} /> : null}
          ...
          {node.kind === 'imported' ? <>
            ...
            <div className="studio-subsection"><div className="studio-section-title"><span>DEBUG VIEW</span><small>EDITOR ONLY</small></div><div className="studio-toggle-grid"><label><input type="checkbox" checked={node.debug.bounds} onChange={(event) => onDebugPatch({ bounds: event.currentTarget.checked })} /> Bounds</label><label><input type="checkbox" checked={node.debug.axes} onChange={(event) => onDebugPatch({ axes: event.currentTarget.checked })} /> Axes</label><label><input type="checkbox" checked={node.debug.skeleton} onChange={(event) => onDebugPatch({ skeleton: event.currentTarget.checked })} /> Skeleton</label></div></div>
            {inspection?.animations.length ? <div className="studio-subsection"><div className="studio-section-title"><span>ANIMATION</span><small>{inspection.animations.length} CLIPS</small></div><label className="studio-field"><span>Clip</span><select value={node.animation.clip ?? ''} onChange={(event) => onAnimationPatch({ clip: event.currentTarget.value || undefined, playing: Boolean(event.currentTarget.value) })}><option value="">No clip</option>{inspection.animations.map((clip) => <option key={clip.name} value={clip.name}>{clip.name} · {clip.duration.toFixed(2)}s</option>)}</select></label><div className="studio-toggle-grid"><label><input type="checkbox" checked={node.animation.playing} disabled={!node.animation.clip} onChange={(event) => onAnimationPatch({ playing: event.currentTarget.checked })} /> Play</label><label><input type="checkbox" checked={node.animation.loop} disabled={!node.animation.clip} onChange={(event) => onAnimationPatch({ loop: event.currentTarget.checked })} /> Loop</label></div><label className="studio-range-field"><span>Speed <output>{node.animation.speed.toFixed(2)}×</output></span><input type="range" min="0.05" max="4" step="0.05" value={node.animation.speed} onChange={(event) => onAnimationPatch({ speed: Number(event.currentTarget.value) })} /></label></div> : null}
            {inspection?.materialSlots.length ? <div className="studio-subsection"><div className="studio-section-title"><span>PBR + TEXTURES</span><small>{inspection.materialSlots.length} SLOTS</small></div>{inspection.materialSlots.map((slot) => <MaterialEditor key={slot.id} slot={slot} override={node.materialOverrides[slot.id]} onPatch={(patch) => onMaterialPatch(slot.id, patch)} onTextureImport={(channel, file) => onTextureImport(slot.id, channel, file)} onTextureReference={(channel, reference) => onTextureReference(slot.id, channel, reference)} />)}</div> : null}
            <div className="studio-subsection studio-export-profile"><div className="studio-section-title"><span>LOCAL DELIVERY</span><small>NO CLOUD</small></div><label className="studio-field"><span>Export profile</span><select value={exportProfile} onChange={(event) => setExportProfile(event.currentTarget.value as StudioGlbExportProfile)}>{Object.entries(studioGlbExportProfiles).map(([key, item]) => <option key={key} value={key}>{item.label}</option>)}</select></label><p className="studio-action-note">{profile.note}</p><div className="studio-delivery-actions"><button className="studio-primary-action" type="button" onClick={() => onExportGlb(exportProfile)}>EXPORT WEB GLB</button><button className="studio-primary-action" type="button" onClick={() => onExportComponent(exportProfile)}>R3F COMPONENT ZIP</button></div><p className="studio-action-note">Component ZIP includes the exported GLB, typed R3F source, exact dependency pins, preset metadata, quality contract and MIT license. No Draco / Meshopt / KTX2 claim is made.</p></div>
          </> : null}
