...
function ExecutiveBriefcase(){const handle=useMemo(()=>new THREE.CatmullRomCurve3([new THREE.Vector3(-.38,.62,0),new THREE.Vector3(-.32,.95,0),new THREE.Vector3(.32,.95,0),new THREE.Vector3(.38,.62,0)]),[]);return <group position={[0,-.5,0]} rotation={[0,-.32,0]}>
  <RoundedBox args={[2.15,1.35,.55]} radius={.18} smoothness={8}><meshPhysicalMaterial color="#3d2a23" roughness={.4} clearcoat={.18} sheen={.28} /></RoundedBox>
  ...
  {[-.58,.58].map(x=><group key={x} position={[x,.12,.32]}><RoundedBox args={[.28,.22,.06]} radius={.04} smoothness={4}><meshPhysicalMaterial color="#b2935e" metalness={.92} roughness={.18} /></RoundedBox><mesh position={[0,.02,.05]}><boxGeometry args={[.09,.05,.03]} /><meshBasicMaterial color="#e2c47b" /></mesh></group>)}
