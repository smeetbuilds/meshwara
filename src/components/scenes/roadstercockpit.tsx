...
function RoadsterCockpit(){return <group position={[0,-.65,0]} rotation={[.05,-.4,0]} scale={.9}>
  <CurvedBox args={[3.3,.55,2.1]} radius={.22} smoothness={6} position={[0,-.2,0]}><meshPhysicalMaterial color="#512e25" metalness={.62} roughness={.24} clearcoat={.7}/></CurvedBox>
  {[-.68,.68].map(x=><group key={x} position={[x,.52,-.18]}><CurvedBox args={[.82,1.15,.78]} radius={.2} smoothness={6}><meshPhysicalMaterial color="#c1aa8d" roughness={.34}/></CurvedBox><CurvedBox args={[.72,.72,.68]} radius={.18} smoothness={6} position={[0,.78,-.12]}><meshPhysicalMaterial color="#bfa78a" roughness={.34}/></CurvedBox></group>)}
