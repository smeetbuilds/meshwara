...
function PrecisionController(){return <group rotation={[.15,-.3,-.05]}>
  <CurvedBox args={[2.7,1.38,.58]} radius={.42} smoothness={6}><meshPhysicalMaterial color="#d8d4cb" roughness={.28} clearcoat={.45}/></CurvedBox>
  {[-.9,.9].map((x,i)=><CurvedBox key={x} args={[.78,1.62,.52]} radius={.35} smoothness={5} position={[x,-.42,-.05]} rotation={[0,0,i?-.16:.16]}><meshPhysicalMaterial color="#d8d4cb" roughness={.3}/></CurvedBox>)}
