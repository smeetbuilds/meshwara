...
function DiningTable(){
  return <group position={[0,-.72,0]} rotation={[0,-.36,0]}>
    <CurvedBox args={[3.45,.22,1.75]} radius={.09} smoothness={6} position={[0,1.45,0]}><meshPhysicalMaterial color="#9a7a58" roughness={.58} clearcoat={.12}/></CurvedBox>
    <CurvedBox args={[3.12,.08,1.48]} radius={.035} smoothness={4} position={[0,1.31,0]}><meshStandardMaterial color="#6d523b" roughness={.68}/></CurvedBox>
    {[-1.28,1.28].map(x=>[-.56,.56].map(z=><group key={`${x}-${z}`} position={[x,.68,z]}>
      <CurvedBox args={[.16,1.38,.16]} radius={.04} smoothness={4}><meshPhysicalMaterial color="#25282a" metalness={.78} roughness={.24}/></CurvedBox>
