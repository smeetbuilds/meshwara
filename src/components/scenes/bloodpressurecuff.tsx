...
function BloodPressureCuff(){const hose=useMemo(()=>new THREE.CatmullRomCurve3([new THREE.Vector3(.6,.0,.25),new THREE.Vector3(1.2,-.2,.1),new THREE.Vector3(1.45,.15,-.25),new THREE.Vector3(1.15,.55,-.45)]),[]);return <group position={[0,-.45,0]} rotation={[0,-.34,0]}>
  <CurvedBox args={[1.65,.88,1.15]} radius={.16} smoothness={5}><meshStandardMaterial color="#e2e3df" roughness={.4}/></CurvedBox><mesh position={[-.18,.08,.59]}><planeGeometry args={[.82,.34]}/><meshBasicMaterial color="#466a6c"/></mesh><mesh position={[.48,-.18,.62]}><circleGeometry args={[.13,28]}/><meshBasicMaterial color="#6aa68d"/></mesh>
