...
const trunkPath:Array<[number,number,number]>=[[0,-1.35,0],[.02,-.7,.02],[-.08,-.08,.04],[.1,.58,.02],[.16,1.18,-.04]]
const branches=[[[.02,.05,.02],[-.5,.42,.08],[-1.0,.72,.15]],[[.1,.4,0],[.58,.68,-.05],[1.08,.92,-.12]],[[.13,.72,-.03],[-.25,1.05,-.12],[-.5,1.38,-.18]],[[.14,.82,-.02],[.48,1.18,.05],[.78,1.42,.1]]] as Array<Array<[number,number,number]>>
const leaves=[[-1.05,.76,.16,-.4],[-.72,.58,.1,.25],[-.48,1.36,-.17,-.6],[-.18,1.25,-.08,.45],[.48,1.2,.06,-.2],[.78,1.44,.1,.55],[1.08,.94,-.12,-.35],[.24,1.5,.02,.18]] as Array<[number,number,number,number]>
...
const leafColors=['#a74d35','#8b7337','#bd6a3f']

export default function JapaneseMapleStudy(){return <group position={[0,-.1,0]} rotation={[0,-.24,0]}>
  <LoftSurface stations={trunkBody} rotation={[0,0,.08]}><meshStandardMaterial color="#62402d" roughness={.86}/></LoftSurface>
  <SplineTube points={trunkPath} radius={.075} tubularSegments={44} radialSegments={9}><meshStandardMaterial color="#62402d" roughness={.9}/></SplineTube>
