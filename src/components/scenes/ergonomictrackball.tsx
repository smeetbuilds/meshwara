...
function ErgonomicTrackball(){
  return <group rotation={[.12,-.36,-.04]}>
    <RoundedBox args={[2.2,.58,1.78]} radius={.32} smoothness={8}><meshPhysicalMaterial color="#303537" roughness={.32} clearcoat={.35}/></RoundedBox>
    ...
    {[.24,.58,.86].map((x,i)=><RoundedBox key={x} args={[.42,.09,.55]} radius={.12} smoothness={5} position={[x,.34,-.18+i*.2]} rotation={[0,0,-.08+i*.04]}><meshPhysicalMaterial color="#4a5052" roughness={.33}/></RoundedBox>)}
    ...
    <RoundedBox args={[.48,.055,.2]} radius={.05} smoothness={4} position={[.48,.35,.7]}><meshStandardMaterial color="#1c2021" roughness={.4}/></RoundedBox>
