...
function SmartHomeHub(){return <group rotation={[.08,-.35,-.02]}>
  <RoundedBox args={[1.8,.82,1.8]} radius={.34} smoothness={8}><meshPhysicalMaterial color="#e2e3df" roughness={.32} clearcoat={.48}/></RoundedBox>
  <RoundedBox args={[1.42,.035,1.42]} radius={.24} smoothness={7} position={[0,.43,0]}><meshPhysicalMaterial color="#f0f1ee" roughness={.28}/></RoundedBox>
  ...
  {[[-.92,0,0],[.92,0,0],[0,0,-.92]].map((p,i)=><RoundedBox key={i} args={[i===2?.5:.12,.14,i===2?.12:.46]} radius={.04} smoothness={4} position={p as [number,number,number]}><meshPhysicalMaterial color="#3a4143" metalness={.35} roughness={.3}/></RoundedBox>)}
