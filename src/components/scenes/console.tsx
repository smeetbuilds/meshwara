...
function TravertineConsole() {
  return (
    <group rotation={[0.03, -0.38, 0]} position={[0, -0.42, 0]}>
      <CurvedBox args={[3.05, 0.28, 0.86]} radius={0.09} smoothness={6} position={[0, 0.72, 0]}>
        <meshPhysicalMaterial color="#cbbca6" roughness={0.72} clearcoat={0.08} />
      </CurvedBox>
      {[-1.1, 1.1].map((x) => (
        <CurvedBox key={x} args={[0.38, 1.68, 0.7]} radius={0.08} smoothness={6} position={[x, -0.23, 0]}>
          <meshPhysicalMaterial color="#c4b49d" roughness={0.76} />
        </CurvedBox>
      ))}
      <CurvedBox args={[1.34, 0.12, 0.54]} radius={0.04} smoothness={4} position={[0, 0.47, 0.02]}>
