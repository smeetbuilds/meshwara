...
export function sanitizeStudioTimeline(value: unknown): StudioTimelineState {
  ...
  const snapRange = (time: number) => clamp(Math.round(time * fps) / fps, 0, duration)
  const rangeStart = snapRange(Math.min(rawRangeStart, rawRangeEnd))
  const rangeEnd = snapRange(Math.max(rawRangeStart, rawRangeEnd))
